...
import type { EasyInputMessage, Response, ResponseOutputMessage } from 'openai/resources/responses/responses';
...
class OpenAiResponsesApi implements AiChatApi {
  ...
  readonly sendMessage = (author: ChatApiAuthor, message: string) => {
    ...
    return from(this._client.responses.create({
      model: this._context,
      input: this.toOpenAiInput(conversation)
    })).pipe(
      map((response) => {
        const responseText = this.toResponseText(response);
  ...
  private readonly toOpenAiInput = (conversation: ChatApiMessage[]): EasyInputMessage[] => {
    return conversation.map((message) => ({
      role: message.direction === 'out' ? 'user' : 'assistant',
      content: message.message
    }));
