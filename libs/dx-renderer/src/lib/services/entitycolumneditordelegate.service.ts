...
export class EntityColumnEditorDelegateService implements ColumnEditorDelegateService {
  ...
  constructor(
    ...
    combineLatest([
      ...
      if (lookups && getGenericLookupByIdentifier && editorValueChanged) {
        const preparedColumn = cloneDeep(this.cell.column.editorOptions) as GridLayoutColumn;

        this._valueChanged$.next((e, value) => {

          const editUtil = {
            getEditorOption: (dataMember, option) => dataMember === this.identifier ? e.getOption(option) : undefined,
            setEditorOption: (dataMember, option, value) => dataMember === this.identifier && e.setOption(option, value),
          ...
          editorValueChanged(this.cell.column.allowEditing ? EditModes.EDIT : EditModes.VIEW, this.row, editUtil, this.identifier, value as ValueType);
        });
