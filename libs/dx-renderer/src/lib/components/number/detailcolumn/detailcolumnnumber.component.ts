...
@Component({
    selector: 'ballware-detail-column-number',
    templateUrl: './detailcolumnnumber.component.html',
    styleUrls: ['./detailcolumnnumber.component.scss'],
    imports: [CommonModule, DxNumberBoxModule, DxValidatorModule]
})
export class DetailColumnNumberComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.detailItem && this.identifier) {
            this.value = get(this.detailItem, this.identifier) as number;
        }
