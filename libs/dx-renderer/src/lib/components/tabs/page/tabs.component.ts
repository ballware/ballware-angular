...
import { PageLayoutItemComponent } from '../../layout';
...
export class PageLayoutTabsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.panels =
      this.layoutItem?.items?.filter((item) => item.type === 'tab') ?? [];

    const identifier = (this.layoutItem?.options?.itemoptions as TabsOptions)
      ?.identifier;
