...
@Component({
    selector: 'ballware-crud-editpopup',
...
export class CrudEditPopupComponent implements OnInit {
  ...
  constructor(
    ...
    this.applyAndContinue$.pipe(
      ...
      if (item && (!validator || validator())) {
        this.apply?.(this.editService.editUtil(), item, true);
      }
    });
      ...
      if (item) {
        if (validator) {
          validator().subscribe((isValid) => {
            if (isValid) {
              this.apply?.(this.editService.editUtil(), item, false);
            }
          });
        } else {
          this.apply?.(this.editService.editUtil(), item, false);
