...
import { CrudForeignEditPopupComponent } from '../foreigneditpopup/foreigneditpopup.component';
...
@Component({
    ...
    imports: [CommonModule, I18NextPipe, DxPopupModule, DxFileUploaderModule, DxActionSheetModule, CrudForeignEditPopupComponent, CrudEditPopupComponent, EditLayoutComponent]
})
export class CrudActionsComponent implements OnInit {
  ...
  constructor(
    ...
    this.displayName$ = this.metaService.displayName$.pipe(
      takeUntilDestroyed(this.destroy)
    );
    ...
    this.crudService.itemDialog$.pipe(
      ...
      this.sanitizedExternalEditorUrl = this.itemDialog?.externalEditor ? this.domSanitizer.bypassSecurityTrustResourceUrl(this.itemDialog.item as string) : undefined;
    });

    this.crudService.removeDialog$.pipe(
      takeUntilDestroyed(this.destroy)
    ).subscribe((removeDialog) => {
      this.removeDialog = removeDialog;
    });

    this.crudService.importDialog$.pipe(
      takeUntilDestroyed(this.destroy)
    ).subscribe((importDialog) => {
      this.importDialog = importDialog;
    });

    this.crudService.detailColumnEditDialog$.pipe(
      takeUntilDestroyed(this.destroy)
  ...
  ngOnInit(): void {
    this.crudService.selectAddSheet$.pipe(
      takeUntilDestroyed(this.destroy),
      withLatestFrom(this.crudService.currentInteractionTarget$)
    ...
    this.crudService.selectPrintSheet$.pipe(
      takeUntilDestroyed(this.destroy),
      withLatestFrom(this.crudService.currentInteractionTarget$)
    ...
    this.crudService.selectActionSheet$.pipe(
      takeUntilDestroyed(this.destroy),
      withLatestFrom(this.crudService.currentInteractionTarget$)
    ...
    this.crudService.selectExportSheet$.pipe(
      takeUntilDestroyed(this.destroy),
      withLatestFrom(this.crudService.currentInteractionTarget$)
    ...
    this.crudService.selectImportSheet$.pipe(
      takeUntilDestroyed(this.destroy),
      withLatestFrom(this.crudService.currentInteractionTarget$)
