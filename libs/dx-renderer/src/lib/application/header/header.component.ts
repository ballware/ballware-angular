...
import { IDENTITY_SERVICE, IdentityService, ResponsiveService, SCREEN_SIZE, TENANT_SERVICE, TenantService, TOOLBAR_SERVICE, ToolbarService } from '@ballware/meta-services';
...
export class ApplicationHeaderComponent extends WithDestroy() {
  ...
  constructor(private responsiveService: ResponsiveService, private translationService: I18NextPipe, @Inject(IDENTITY_SERVICE) private identityService: IdentityService, @Inject(TENANT_SERVICE) private tenantService: TenantService, @Inject(TOOLBAR_SERVICE) private toolbarService: ToolbarService) {
    super();
