...
export type DetailColumnConfigurationFactory = <ColumnType extends TreeListColumn | DataGridColumn>(
  column: GridLayoutColumn,
  dataMember: string,
  lookups: Record<string, LookupElementType>,
...
export interface ColumnConfigurationRegistry {
  ...
  resolveEntityColumnConfiguration<ColumnType extends TreeListColumn | DataGridColumn>(
    identifier: string,
    column: GridLayoutColumn,
    lookups: Record<string, LookupElementType>,
    lookupParams: Record<string, unknown>
  ...
  resolveDetailColumnConfiguration<ColumnType extends TreeListColumn | DataGridColumn>(
    identifier: string,
    column: GridLayoutColumn,
    dataMember: string,
    lookups: Record<string, LookupElementType>,
...
export class DefaultColumnConfigurationRegistry implements ColumnConfigurationRegistry {
  ...
  private readonly _entityConfigurationFactories = new Map<string, EntityColumnConfigurationFactory>();
  private readonly _detailConfigurationFactories = new Map<string, DetailColumnConfigurationFactory>();

  registerEntityColumnConfigurationFactory(identifier: string, factory: EntityColumnConfigurationFactory): void {
    this._entityConfigurationFactories.set(identifier, factory);
  }

  registerDetailColumnConfigurationFactory(identifier: string, factory: DetailColumnConfigurationFactory): void {
  ...
  resolveEntityColumnConfiguration<ColumnType extends TreeListColumn | DataGridColumn>(
    ...
    if (!factory) {
      throw new Error(`No entity column configuration factory registered for identifier: ${identifier}`);
    }
  ...
  resolveDetailColumnConfiguration<ColumnType extends TreeListColumn | DataGridColumn>(
    ...
    if (!factory) {
      throw new Error(`No detail column configuration factory registered for identifier: ${identifier}`);
    }

    return factory(column, dataMember, lookups, parentItem);
  }
}
