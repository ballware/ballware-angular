...
ENV_JSON="$(jq --compact-output --null-input 'env | with_entries(select(.key | startswith("BALLWARE_")))')"
...
sed -i "s/<noscript id=\"env-insertion-point\"><\/noscript>/<script>ENV=Object.assign(ENV, ${ENV_JSON_ESCAPED})<\/script>/g" /var/www/index.html
